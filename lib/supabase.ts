...
if (isValidUrl(supabaseUrl) && supabaseKey) {
    ...
    console.warn("⚠️ Supabase URL/Key missing. Offline Mode Active.");
    console.warn("URL:", supabaseUrl ? "Set" : "Missing");
    console.warn("KEY:", supabaseKey ? "Set" : "Missing");
    ...
    const dummyBuilder = {
        ...
        then: function (resolve: any) {
            ...
            resolve({
                data: [],
                error: { message: "Supabase disconnected (Offline Mode) - Check console for missing keys" },
                count: 0,
                status: 200,
                statusText: 'OK'
            });
