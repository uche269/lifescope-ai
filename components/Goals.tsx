...
const Goals: React.FC<GoalsProps> = ({ goals, setGoals }) => {
  ...
  const toggleActivity = (goalId: string, activityId: string) => {
    if (editingActivityId === activityId) return; // Don't toggle if editing
    setGoals(goals.map(g => {
      if (g.id !== goalId) return g;
      const updatedActivities = g.activities.map(a => 
  ...
  const saveNewActivity = (goalId: string) => {
    ...
    setGoals(goals.map(g => {
        ...
        return {
            ...
            activities: [...g.activities, { 
                ...
                isCompleted: false, 
                frequency: newActivityFreq as any 
            }]
        }
    }));
  ...
  const deleteActivity = (goalId: string, activityId: string) => {
      setGoals(goals.map(g => {
          if (g.id !== goalId) return g;
          return {
              ...g,
  ...
  const saveActivityName = (goalId: string, activityId: string) => {
      setGoals(goals.map(g => {
          if (g.id !== goalId) return g;
          return {
              ...g,
